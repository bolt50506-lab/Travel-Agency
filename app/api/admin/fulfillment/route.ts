...
export async function GET(req: NextRequest) {
  ...
  }catch(err){if(err instanceof Error&&err.message==='UNAUTHORIZED_ADMIN')return errorResponse('Staff access required','FORBIDDEN',403);console.error(err);return errorResponse('Unable to load fulfillment queue','INTERNAL_ERROR',500);}
}
