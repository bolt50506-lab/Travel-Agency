            ...
            <div className="space-y-4">
              ...
              <Card className="p-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <Select value={paymentMethod} onValueChange={(v) => setPaymentMethod(v as typeof paymentMethod)}>
                      ...
                      <SelectContent>
                        {actorRole === 'customer' && (
                          <SelectItem value="wallet" disabled={walletBalance === null || walletBalance < Number(selectedRoom.totalPrice.amount)}>
                ...
                {actorRole === 'customer' && walletBalance !== null && (
                  <div className={cn('mt-3 rounded-lg border p-3 text-xs', walletBalance >= Number(selectedRoom.totalPrice.amount) ? 'border-primary/30 bg-primary/5' : 'border-amber-400/40 bg-amber-50/50')}>
                    <span className="font-semibold">Wallet balance:</span> PKR {walletBalance.toLocaleString('en-PK')}
                    {walletBalance < Number(selectedRoom.totalPrice.amount) && ' — insufficient for this booking.'}
                  </div>
                )}
              ...
              <Button size="lg" className="w-full" onClick={handlePayment} disabled={paymentProcessing}>
                {paymentProcessing ? (
                  ...
                  <>{paymentMethod === 'wallet' ? 'Pay from Wallet' : `Pay ${formatPrice(selectedRoom.totalPrice.amount, selectedRoom.totalPrice.currency)}`}</>
                )}
              </Button>
            </div>
          )}
