...
export default function AdminModuleManager({ module, title, description }: { module: string; title: string; description: string }) {
  ...
  async function create(){
    try{const r=await fetch('/api/admin/modules',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({module,data:form})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to create');toast.success('Record created');setForm({});setShowForm(false);load();}catch(e){toast.error(e instanceof Error?e.message:'Unable to create');}
  }
  ...
  return <div className="space-y-6">
    ...
    {showForm && <Card className="p-5">
      <h2 className="font-semibold mb-4">Create {title.replace(/s$/,'')}</h2>
      ...
      <div className="mt-4 flex gap-2"><Button onClick={create}>Save</Button><Button variant="outline" onClick={()=>setShowForm(false)}>Cancel</Button></div>
    </Card>}
    ...
    <Card className="p-4">
      ...
      <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left">{data.fields.slice(0,8).map(f=><th key={f} className="px-3 py-2 font-medium capitalize">{f.replaceAll('_',' ')}</th>)}</tr></thead><tbody>{data.rows.map((row:any)=><tr key={row.id} className="border-b last:border-0"><>{data.fields.slice(0,8).map(f=><td key={f} className="px-3 py-3 max-w-[240px] truncate">{String(row[f]??'—')}</td>)}</></tr>)}</tbody></table></div>}
    </Card>
  </div>;
}
